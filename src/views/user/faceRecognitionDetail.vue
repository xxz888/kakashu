...
<template>
  <div>
    ...
    <div class="main ignore">
      <div class="content">
              ...
              <div class="idCardImg">
                <div class="idcard">
                  <label
                    ...
                    class="idcardz_bg"
                    :style="
                      imgz ? 'background: url(' + imgz + ') no-repeat;' : ''
                    "
                  ></label>
                  <input
                    name="headFile"
              ...
              <div class="idCardImg">
                <div class="idcard">
                  <label
                    ...
                    class="idcardf_bg"
                    :style="
                      imgf ? 'background: url(' + imgf + ') no-repeat;' : ''
                    "
                  ></label>
                  <input
                    name="backFile"
              ...
              <div class="idCardImg">
                <div class="idcard">
                  <label
                    ...
                    class="idcardl_bg"
                    :style="
                      imgs ? 'background: url(' + imgs + ') no-repeat;' : ''
                    "
                  ></label>
                  <input
                    name="faceFile"
        ...
        <form>
          ...
          <van-field
            ...
            label-width="60px"
            maxlength="18"
            v-model="userMessage.number"
            placeholder="请扫描或者输入身份证号"
            type="text"
            label="身份证号"
            required
          />
      ...
      <div class="submit make_plan_btn theme_btn color_fff" @click="submit()">
        实名认证
      </div>
      <van-uploader
        id="ren"
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      submitAutonym(
        this.userMessage.number,
        this.userMessage.name,
        // this.base64String,
        this.userMessage.address,
        this.empowerToken
      ).then((res) => {
        if (res.resp_code == "000000") {
          userInfoQuery(
            this.empowerToken ? this.empowerToken : this.token
          ).then((res) => {
            ...
            if (res.resp_code == "000000") {
             ...
             if (this.empowerToken) {
              this.$router.push({
                name: "depositCardAdd",
                params: { type: 1 },
                query: { empowerToken: this.empowerToken ,isBackAgentView:this.isBackAgentView},
              }); //绑卡页面
            } else {
    ...
    _updateRealnameImage(rst) {
      ...
      updateRealnameImage(rst).then((res) => {
        ...
        if (res.resp_code == "000000") {
          if (
            res.result.data.info.name != null &&
            res.result.data.info.name != ""
          ) {
