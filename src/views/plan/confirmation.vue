<template>
  <div>
    ...
    <div class="warpper">
      <div class="upcreditcard_title">
        信息加密处理，仅用于银行验证，请验证您本人的银行卡
      </div>
      <div class="card_box">
        <div class="card_item">
          <van-field
          ...
          <van-field
            ...
            maxlength="3"
            readonly="readonly"
            placeholder="请输入(卡背面CVN2后三位数字)"
            clearable
          />
          <van-field
            v-model="card.expiredTime"
            label="有效期"
            maxlength="4"
            readonly="readonly"
            placeholder="请输入(如09/22输入0922)"
            clearable
          />
          <van-field
            readonly
      ...
      <div class="upcreditcard_btn_box">
        <van-button
          @click.native="channel.needCode ? confirm() : getCode()"
          type="primary"
          class="upcreditcard_btn"
          round
          >确认</van-button
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    topClose() {
      // 跳转刷卡详情页 关闭新手引导 清除定时器
      this.$store.commit("Loading");
      clearInterval(this.conti);
      let year = new Date().getFullYear();
      let month = new Date().getMonth() + 1;
      quickOrderQuery(this.brandId, this.userId, year, month, "20").then(
        (res) => {
          this.$store.commit("closeLoading");
    ...
    continueFun() {
      //如果返回999998每秒请求一次结果 请求5秒 直到返回000000
...
.upcreditcard_title {
  height: 40px;
  background: rgba(254, 161, 51, 0.3);
  line-height: 40px;
  width: 100%;
  color: #f5552a;
  font-size: 12px;
...
.box .bg {
  background: url("https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/addcard/security_code.png")
    center no-repeat;
