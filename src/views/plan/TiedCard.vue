...
import { verifyCodeApply, verifyCodeConfirm, balanceBindCard, balanceBindCardConfirm } from '@/api/online'
  ...
  methods: {
    ...
    getCode() {
      ...
      if(this.params){
        ...
        verifyCodeApply(data).then(res => {
          if(res.resp_code == '000000'){
            this.$toast({ message: '绑卡成功', position: 'bottom' })
            listener.$emit('backFromBindCard', 1)
            history.back()
          }else{
            this.$toast({ message: res.resp_message, position: 'bottom' })
    ...
    confirm() {
      ...
      if(this.params){
        this.params['smsMsg'] = this.sms
        balanceBindCardConfirm(this.params).then(res => {
          if(res.resp_code == '000000'){
            listener.$emit('backFromBindCard', 1)
            history.back()
          }else{
        ...
        verifyCodeConfirm({ channelTag: this.channel.channelTag, bankCard: this.card.cardNo, smsMsg: this.sms, idCard: this.card.idcard }).then(res => {
          if(res.resp_code == '000000'){
            history.back()
          }else{
            this.$toast({ message: res.resp_message, position: 'bottom' })
