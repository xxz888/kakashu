...
export default {
  ...
  created() {
    ...
    this.empowerToken = this.$route.params.empowerToken
  },
  methods: {
    onClickLeft() {
      this.publicJs.back();
    ...
    update() {
      ...
      upcreditcard(this.userId, this.item.cardNo, this.item.securityCode, this.item.expiredTime, this.item.billDay, this.item.repaymentDay, this.item.creditBlance,this.empowerToken).then(res => {
