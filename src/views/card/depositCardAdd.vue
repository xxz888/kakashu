<template>
  <div>
    ...
    />
    <div class="warpper">
      <div class="deposit_upcreditcard_title">
        请添加本人储蓄卡作为结算卡，该卡作为后续刷卡、提现资金的结算
        账号，户名必须与实名认证姓名一致
      </div>
      <div class="card_box">
        <div class="card_item">
          <div class="card_item_tips theme">
            *建议：工商、农业、交通、招商、民生、华夏、平安等一类卡（不支持二类卡及地方性银行）
          </div>
          <div class="card_item_add">
            <img
              :src="
                imgUrl == '' ? '../../assets/card/add_card_icon.png' : imgUrl
              "
              @click="scanCard()"
              alt=""
            />
          ...
          <van-field
            v-if="userList.realnameStatus == 1"
    ...
    add() {
     let tokenTemp = this.token
      ...
      addBank(
        ...
        this.item.cardNo,
        this.userList.idcard,
        this.item.phone.replace(/\s/g, ""),
        0,
        "",
        "",
        "",
