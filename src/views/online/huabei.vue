...
<template>
  <div class="online">
    <div class="header theme_bg">
      <van-nav-bar class="online_nav" title="花呗收款" left-arrow fixed :border='false' @click-left="onClickLeft"/>
      <div class="online_main">
        <div class="online_main_head">
          <span>请输入金额(元)&nbsp;&nbsp;&nbsp;&nbsp;交易时间 8:00-20:00</span>
        </div>
        ...
        <van-cell class="van-ellipsis" :value="cityName" is-link @click="diycity()">
          <!-- 使用 title 插槽来自定义标题 -->
          <template #title>
        ...
        <van-row style="margin-top:20px;margin-bottom: 20px">
          ...
          <van-col :span="20">
            <van-radio-group v-model="fenqi" checked-color="#F71111" icon-size="14" direction="horizontal">
    ...
    <city ref="verificationMethod" :channel="channelTag" />
...
import {
  ...
  Row,
  Col,
  Cell, RadioGroup, Radio
} from 'vant'
import {userBankAndNature, bankcardIsdefault, getchannelBybankcardNew, newFacadeTopup, huabeiPayApi} from '@/api/online'
  ...
  methods: {
    ...
    handleAmount() {
      if (this.level.diffRate) {
        return (88 - this.level.diffRate.huabeiFast * 10000).toFixed(0)
      } else {
        return 0
      }
    },
    ...
    setChannel(recommend) {
      ...
      huabeiPayApi({
        ...
        debitBankCard: this.defJieCardList[0].cardNo,
        userName: this.defJieCardList[0].userName,
