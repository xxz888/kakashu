...
let toFixedPositive = value =>{
    if(value==null){
        return 0.00
    }else{
        return Math.abs(value).toFixed(2) == 0 ? '0.00': Math.abs(value).toFixed(2)
    }
     
}
export { toFixedPositive }
...
let agentStatus = value => {
  return '运行中的计划:' + value.runCount + ' ' + ' ' + '' + '异常的计划:' + value.failCount;
}
export { agentStatus }